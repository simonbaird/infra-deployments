...
A "go make some coffee" one liner for CRC:
    ...
    cd $(git rev-parse --show-toplevel); crc delete; crc start; `crc console --credentials | tail -1 | cut -d\' -f2`; oc new-project application-service; oc new-project enterprise-contract-service; source hack/preview.env; DEPLOY_ONLY=build,enterprise-contract hack/bootstrap-cluster.sh preview
